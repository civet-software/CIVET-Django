...
#    print('DD1:',curfilename)
    ...
    for row in CIV_template.tempdata: # needs to be modified since tempdata is just a \t and \n delimited string
        writer.writerow(row)
...
def read_template(request):
    """ main routine for setting up a template: reads a file, checks for errors, and then either renders the form or 
        lists the errors """
    global ActiveTemplate, basic_mode
...
def download_basic_data(request):
    CIV_template.save_to_TempData(request)
    return render(request,'djciv_data/download_data.html',{'filename' :CIV_template.defaultfilename})


def reset_data(request):
    CIV_template.create_TempData_header()
    if CIV_template.basic_mode:
        return render(request,'djciv_data/basic_form.html',  {'form_content' : ActiveTemplate})
    else:
        return render(request,'djciv_data/file_select.html',{})
...
#    print('Styles:\n',termstyles)
#    print('Edited:\n',cktext)
    context['markedtext'] = cktext
    context['newterm'] = termstyles
    return render(request,'djciv_data/civet_coder.html',context)
...
def save_and_new(request):
    CIV_template.save_case(request, '')
    return render(request,'djciv_data/select_collection.html',{'files' : CollectionList})
    
"""def save_new_case(request):
    CIV_template.save_case(request, '')
    return HttpResponse('Successful save')"""
